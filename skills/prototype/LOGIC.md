...
Record what the prototype demonstrated and keep the HTML file runnable. If production implementation is requested, adapt the validated reducer, machine, or functions into the real module with appropriate verification. Preserve a pointer to the prototype wherever the project records such decisions; create commits or published branches only within the user's authorization.
